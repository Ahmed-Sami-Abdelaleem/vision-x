...
class Tracker:
    ...
    def assign_ball_possession(self, tracks, ball_threshold=50):
        for frame_num in range(len(tracks['ball'])):
            ball_data = tracks['ball'][frame_num].get(1)
            if not ball_data: continue
    ...
    def interpolate_ball_positions(self, ball_positions):
        # Extract bbox coordinates, handling cases where ball is not detected
        ball_bboxes = []
        for frame in ball_positions:
            if 1 in frame:  # Check if ball was detected in this frame
                bbox = frame[1].get('bbox', [])
        ...
        # Reconstruct the ball positions in the original format
        ball_positions = []
        for _, row in df_ball_positions.iterrows():
            ball_positions.append({1: {"bbox": [row['x1'], row['y1'], row['x2'], row['y2']]}})
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        for frame_num, detection in enumerate(detections):
            ...
            for frame_detection in detection_supervision:
                ...
                if cls_id == cls_names_inv['ball'] and 1 not in tracks["ball"][frame_num]:
    ...
    def draw_ellipse(self, frame, bbox, color, track_id=None):
        ...
        if any(np.isnan(coord) for coord in bbox):
            return frame  # Skip drawing if bbox is invalid
        ...
        try:
            ...
            return frame  # Skip drawing if any coordinate is invalid
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        cv2.putText(frame, f"Team 1 Ball Control: {team_1_percent:.2f}%", 
                    (1400, 900), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
        cv2.putText(frame, f"Team 2 Ball Control: {team_2_percent:.2f}%", 
    ...
    def draw_annotations(self, video_frames, tracks, team_ball_control):
        ...
        for frame_num, frame in enumerate(video_frames):
            ...
            for track_id, player in player_dict.items():
                # Ensure color is a valid BGR tuple
                color = player.get("team_color", (0, 0, 255))  # Default to red if no color
                if not isinstance(color, tuple) or len(color) != 3:
                    color = (0, 0, 255)  # Fallback to red
            ...
            for track_id, ball in ball_dict.items():
                frame = self.draw_traingle(frame, ball["bbox"], (0, 255, 0))  # Green
